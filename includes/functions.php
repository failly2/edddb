...
add_filter( 'edd_metabox_fields_save', 'edd_db_register_metabox_inputf', 10, 1 );
...
function edd_db_display_metbox_inputf( $post_id ) {
	if ( empty( $post_id ) ) {
		return;
	}
	
	$post_id = absint( $post_id );
	$current_setting = get_post_meta( $post_id, '_edd_db_display_badgef', true );
	$output  = '<p><strong>' . __( 'Future Badges', 'edd-db-txtt' ) . '</strong></p>';
	$output .= '<p><input' . checked( '1', $current_setting, false ) . ' type="checkbox" id="_edd_db_display_badgef" name="_edd_db_display_badgef" value="1" />';
	$output .= '<label for="_edd_db_display_badgef">' . __( 'Display an indication that the product is fulfilled via download', 'edd-db-txtt' ) . '</label></p>';
...
add_action( 'edd_meta_box_settings_fields', 'edd_db_display_metbox_inputf', 10, 1 );
...
function edd_db_get_badge_string() {
	...
	$badge_text = edd_get_option( 'EDD_Digital_Badge_badge_textt', '[' . __( 'future', 'edd-db-txtt' ) . ']' );
...
function edd_db_is_digital_downloadf( $download_id = 0 ) {
	if ( empty( $download_id ) ) {
		return false;
	}

	$display_badge = get_post_meta( $download_id, '_edd_db_display_badgef', true );

	$is_digital = $display_badge === '1' ? true : false;

	return apply_filters( 'edd_db_is_digital_downloadf', $is_digital, $download_id );
}
...
function edd_db_append_titlef( $title, $post_id ) {
	if ( is_admin() || 'download' !== get_post_type( $post_id ) || edd_is_checkout() ) {
		return $title;
	}
	...
	if ( edd_db_is_digital_downloadf( $post_id ) ) {
		$badge  = edd_db_badge_string();
		$title .= $badge;
	}
...
add_filter( 'the_title', 'edd_db_append_titlef', 10, 2 );
